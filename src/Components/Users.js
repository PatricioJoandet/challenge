		...
		<>
        	...
        	<img className='w-[25%]' src='logo.png' />
				...
				{userData.slice(firstElement, lastElement).map((element) => {
					return(
						<div className="flex flex-col  bg-slate-100 text-black px-3 py-2 rounded-md">
							<div>
				...
				<div className="flex flex-row justify-start gap-3">
					...
					<button onClick={()=>nextPage()} className={currentPage+1> Math.ceil(userData.length/2) ? "bg-[#D3D3D3] px-4 py-1 text-white":"bg-[#d30070] px-4 py-1 text-white"}>Siguiente</button>
